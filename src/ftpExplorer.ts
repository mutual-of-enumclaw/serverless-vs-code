...
export class FtpModel {
		...
		return this.connect().then(client => {
			return new Promise((c, e) => {
				client.list((err: any, list: any) => {
					...
					return c(this.sort(list.map((entry: { name: any; type: string; }) => ({ resource: vscode.Uri.parse(`ftp://${this.host}///${entry.name}`), isDirectory: entry.type === 'd' }))));
				});
			});
		});
	}
	...
	public getChildren(node: FtpNode): Thenable<FtpNode[]> {
		return this.connect().then(client => {
			return new Promise((c, e) => {
				client.list(node.resource.fsPath, (err: any, list: any[]) => {
					if (err) {
						return e(err);
					}
					...
					return c(this.sort(list.map((entry: { name: any; type: string; }) => ({ resource: vscode.Uri.parse(`${node.resource.fsPath}/${entry.name}`), isDirectory: entry.type === 'd' }))));
				});
			});
		});
	}
	...
	public getContent(resource: vscode.Uri): Thenable<string> {
		return this.connect().then(client => {
			return new Promise((c, e) => {
				client.get(resource.path.substr(2), (err: any, stream: { on: (arg0: string, arg1: { (buffer: any): void; (): void; }) => void; }) => {
					if (err) {
						return e(err);
					}

					let string = '';
					(stream as any).on('data', buffer => {
						if (buffer) {
